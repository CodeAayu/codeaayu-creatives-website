window.WEDDING_UPDATES = {
    lastUpdated: '4 April 2026',
    important: {
        tag: 'Travel Guide',
        date: '4 April 2026',
        tone: 'travel',
        title: 'Reaching Pilibanga',
        message: 'The nearest railway station is Pilibanga. If you are flying in, Jaipur and Delhi are the closest airports, each around 350 to 400 km away by road. From either city, a nightly AC sleeper bus is the easiest onward option.',
        highlights: [
            'Pilibanga Station',
            'Jaipur Airport',
            'Delhi Airport',
            'Night AC Sleeper Bus'
        ],
        locations: [
            {
                label: 'Pilibanga Station',
                url: 'https://maps.app.goo.gl/Kn1rFffbhVLrivsSA'
            },
            {
                label: 'Jaipur Airport',
                url: 'https://maps.app.goo.gl/njT3BMLxob7E32pY9'
            },
            {
                label: 'Delhi Airport',
                url: 'https://maps.app.goo.gl/88TvfzqX9YtUxs23A'
            }
        ]
    },
    notices: [
        {
            date: '24-25 April 2026',
            title: 'Logistics Help',
            label: 'Guest Support',
            message: 'If you need help with travel, stay, or arrival coordination in Pilibanga, please reach out to Mr. Ram Ahuja (Groom\'s father) or Aayush.',
            actions: [
                {
                    label: 'Message Ram Uncle',
                    url: 'https://wa.me/919929425899'
                },
                {
                    label: 'Message Aayush',
                    url: 'https://wa.me/918873643965'
                }
            ]
        },
        {
            date: '24-25 April 2026',
            title: 'Travel Note',
            label: 'Guest Travel',
            message: 'Guests travelling by train can plan for Pilibanga, the nearest railway station. Guests travelling by air can come via Jaipur or Delhi and continue onward by nightly AC sleeper bus service.',
            locations: [
                {
                    label: 'Pilibanga Station',
                    url: 'https://maps.app.goo.gl/Kn1rFffbhVLrivsSA'
                },
                {
                    label: 'Jaipur Airport',
                    url: 'https://maps.app.goo.gl/njT3BMLxob7E32pY9'
                },
                {
                    label: 'Delhi Airport',
                    url: 'https://maps.app.goo.gl/88TvfzqX9YtUxs23A'
                }
            ]
        },
        {
            date: '24-25 April 2026',
            title: 'Accommodation Note',
            label: 'Guest Stay',
            message: 'Guest stay has been arranged at Maheshwari Bhawan, Crest Hotel, and Kandhari Hotel. Once you share your RSVP, room allocation and arrival coordination can be planned smoothly.',
            locations: [
                {
                    label: 'Maheshwari Bhawan',
                    url: 'https://maps.app.goo.gl/nuJfAm5ZFHbpg4PM8'
                },
                {
                    label: 'Crest Hotel',
                    url: 'https://maps.app.goo.gl/oHw4HFBgdEn73UJA9'
                },
                {
                    label: 'Kandhari Hotel',
                    url: 'https://maps.app.goo.gl/KxgqGbLgdvKjNRUG6'
                }
            ]
        },
        {
            date: '4 April 2026',
            title: 'Dress Code Note',
            label: 'Guest Attire',
            message: 'The 24 April evening functions are best in western formals, Haldi is best in yellowish shades, and the wedding evening is best in festive Indian occasion wear.'
        },
        {
            date: '24 April 2026',
            title: 'Ring Ceremony & Sangeet',
            label: 'Evening Celebration',
            message: 'For the 24 April evening functions, western formals are the preference. Girls will mostly be in gowns or dresses, and boys in three-piece suits or similar formal wear. This function is in an air-conditioned indoor hall.'
        },
        {
            date: '25 April 2026',
            title: 'Haldi Ceremony',
            label: 'Morning Function',
            message: 'For Haldi, guests are preferred in yellowish shades. Light festive outfits that are comfortable for a morning function will be ideal, and this function is also in an air-conditioned indoor hall.'
        },
        {
            date: '25 April 2026',
            title: 'Wedding Ceremony',
            label: 'Formal Festive',
            message: 'For the wedding ceremony, indo-western is the preference for gents, while lehengas or sarees are preferred for ladies. This celebration is in a garden setting at Swarn Mahal, Rajvi Palace, so comfortable footwear will also help for the evening.'
        }
    ]
};
