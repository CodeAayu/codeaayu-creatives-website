...
function formatWeatherTime(value) {
    ...
    return `${displayHour}:${String(minutes).padStart(2, '0')} ${meridiem} IST`;
}
...
function renderWeddingWeatherPanel(state) {
    const panel = document.getElementById('weddingWeatherPanel');
    if (!panel) return;

    if (state.status === 'loading') {
        panel.innerHTML = '<div class="wedding-weather-panel-loading">Loading live weather for Pilibanga...</div>';
        return;
    }

    if (state.status === 'error') {
        panel.innerHTML = `
            <div class="wedding-weather-panel-error">
                <strong class="wedding-weather-panel-error-title">Live weather is unavailable right now</strong>
                <p class="body-base wedding-weather-panel-error-text">Please refresh the page in a little while to check current conditions in Pilibanga.</p>
            </div>
        `;
        return;
    }

    const forecastMarkup = state.forecast.map(day => `
        <article class="wedding-weather-forecast-card">
            <span class="wedding-weather-forecast-day">${day.label}</span>
            <strong class="wedding-weather-forecast-condition">${day.condition}</strong>
            <div class="wedding-weather-forecast-range">
                <span>High ${Math.round(day.max)}°C</span>
                <span>Low ${Math.round(day.min)}°C</span>
            </div>
            <span class="wedding-weather-forecast-rain">Rain chance: ${Math.round(day.rainChance)}%</span>
        </article>
    `).join('');

    panel.innerHTML = `
        <div class="wedding-weather-panel-topline">
            <span class="wedding-weather-panel-label">Live Update</span>
            <span class="wedding-weather-panel-time">Updated ${state.updatedAt}</span>
        </div>
        <div class="wedding-weather-panel-grid">
            <section class="wedding-weather-current">
                <span class="wedding-weather-current-kicker">Right now in Pilibanga</span>
                <div class="wedding-weather-current-temperature">${Math.round(state.current.temperature)}°C</div>
                <strong class="wedding-weather-current-condition">${state.current.condition}</strong>
                <p class="body-base wedding-weather-current-text">
                    Feels like ${Math.round(state.current.apparentTemperature)}°C with winds near ${Math.round(state.current.windSpeed)} km/h.
                </p>
            </section>
            <section class="wedding-weather-forecast">
                <div class="wedding-weather-forecast-header">
                    <h3 class="heading-medium wedding-weather-forecast-title">Next 3 Days</h3>
                    <p class="body-small wedding-weather-forecast-text">A quick forecast to help with travel and packing.</p>
                </div>
                <div class="wedding-weather-forecast-grid">
                    ${forecastMarkup}
                </div>
            </section>
        </div>
...
async function initWeddingWeather() {
    const panel = document.getElementById('weddingWeatherPanel');
    if (!panel) return;
    ...
    try {
        ...
        const forecast = daily.time.slice(1, 4).map((time, index) => ({
            label: formatWeatherDay(time),
            condition: describeWeatherCode(daily.weather_code[index + 1], true),
            max: daily.temperature_2m_max[index + 1],
            min: daily.temperature_2m_min[index + 1],
            rainChance: daily.precipitation_probability_max[index + 1] ?? 0
        })).filter(day => day.label);

        renderWeddingWeatherPanel({
            status: 'ready',
        ...
        console.error(error);
        renderWeddingWeatherPanel({ status: 'error' });
    }
}
